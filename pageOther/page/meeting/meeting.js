...
import * as constants from '../../../utils/config.js';
/**
...
let userId="";
let isStart=false;
let uuid = `${parseInt(Math.random() * 1000000)}`;
Page({
   ...
   */
  onShow: function() {
    getApp().globalData.norefreshMessage = true; //判断返回到聊天页面需不需要刷新
    let media = this.data.media || [];
    media.forEach(item => {
      if (item.type === 0) {
        //return for pusher
  ...
  onLeave: function() {
    ...
    if (constants.STATIS_FLAG){
      let pp={
        uuid: uuid,
        channel: channel,
        userId: userId
      }
      call.request(`${constants.APP_SERVER}/agora/video/stop.do`, pp, res => {});
    }




    if (constants.REC_FLAG) {
      //保存录制视频
      let param = {
      ...
      call.request(`${constants.AGORA_SERVER}/${constants.APPID}/cloud_recording/resourceid/${resourceId}/sid/${sid}/mode/mix/stop`, param, res => {

        if (res.serverResponse!=null && res.serverResponse.uploadingStatus == "uploaded") {
          let p = {
            userId: userId,
            channel: channel,
            fileList: res.serverResponse.fileList
          };
          call.request(`${constants.APP_SERVER}/agora/video/save.do`, p, res => {
            // if (res.code == 200) {
            //   wx.showToast({
            //     title: '视频保存成功',
            //     icon: 'success',
  ...
  onSubmitLog: function() {
    let page = this;
    let mediaAction = this.isBroadcaster() ? "打开观察模式" : "关闭观察模式"
    wx.showActionSheet({
      itemList: [mediaAction],
      success: res => {
...
//选中放大
  select(e) {
    let medias = this.data.media;
    let id = e.target.dataset.uid;
    if (id == this.data.isSelect) {
      this.setData({
        isSelect: 0
      })
      this.refreshMedia(this.syncLayout(medias));
      return;
    }
    ...
    for (var i = 0; i < medias.length; i++) {
      if (medias[i].uid == id) {
        medias[i].left = 0;
        medias[i].top = 0;
        medias[i].width = this.layouter.containerWidth;
        medias[i].height = this.layouter.containerHeight;
      } else {
        medias[i].width = 0;
        medias[i].height = 0;
      }
      this.refreshMedia(medias);
    }
  },
...
//旋转元素
  onRotate: function () {
    ...
    if (this.data.isRotate){
      this.setData({ isRotate: false, isSelect:0});
      for (var i = 0; i < medias.length; i++) {
          medias[i].rotate = '';
      }
      this.refreshMedia(this.syncLayout(medias));
    }else{
      this.setData({ isRotate: true });
      for (var i = 0; i < medias.length; i++) {
        if (medias[i].uid == this.data.isSelect) {
          // medias[i].left = 0;
          // medias[i].top = 0;
          medias[i].rotate = `transform: rotate(90deg);`;
          medias[i].width = this.layouter.containerWidth ;
          medias[i].height = this.layouter.containerHeight;
        }
        this.refreshMedia(medias);
        ...
        client.join(undefined, channel, uid, () => {
          ...
          if (this.isBroadcaster()) {
            client.publish(url => {
              ...
              console.error('---------------------------------client join channel success----------------------------------------------------');
              ...
              resolve(url);
                ...
                if (constants.REC_FLAG) {
                  ...
                  setTimeout(() => {
                  call.request(`${constants.AGORA_SERVER}/${constants.APPID}/cloud_recording/acquire`, param, res => {
                    if (res.resourceId != null) {
                      ...
                      call.request(`${constants.AGORA_SERVER}/${constants.APPID}/cloud_recording/resourceid/${resourceId}/mode/mix/start`, param2, res => {
                        if (res.sid != null) {
                          sid = res.sid
                        }
                      });
  ...
  subscribeEvents: function(client) {
    ...
    client.on("stream-added", e => {
      ...
      if (!isStart && constants.STATIS_FLAG) {
        isStart=true
        let pp = {
          uuid: uuid,
          channel: channel,
        ...
        call.request(`${constants.APP_SERVER}/agora/video/start.do`, pp, res => { });
      }
