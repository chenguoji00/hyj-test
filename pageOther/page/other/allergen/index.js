...
Page({
  ...
  deleteAllergy(event) {
    ...
    for (let i = 0; i < this.data.AllergyList.length; i++) {
      if (event.currentTarget.dataset.item.title == this.data.AllergyList[i].title) {
        this.data.AllergyList.splice(i, 1);
        this.data.showAllergy.splice(i, 1);
      }
    }
    for (let i = 0; i < this.data.alleryData.length; i++) {
      for (let j = 0; j < this.data.alleryData[e.index-1].children.length; j++) {
        if(e.title == this.data.alleryData[e.index-1].children[j].title){
          this.data.alleryData[e.index-1].children[j].isActive = !this.data.alleryData[e.index-1].children[j].isActive;
        }
    }
  }
    wx.setStorageSync('AllergyList', this.data.AllergyList);
